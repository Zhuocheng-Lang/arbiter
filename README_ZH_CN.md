...
- **app** — CLI 命令执行和启动编排
- **rules** — 规则加载、诊断、解析后模型与匹配
- **platform/linux** — Linux 专属的进程事件流与 scx 调度器探测

其余顶层模块保持单一职责：

- **daemon** 负责运行时编排与信号处理
- **applier** 负责把解析后的规则转换为具体系统调整
- **cli** 与 **config** 只承载输入模型与配置模型

开发者可参考 [docs/architecture.md](docs/architecture.md) 查看新的模块布局和运行时数据流。
